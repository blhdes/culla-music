...
- **Playlist membership chips** — small pills under the artist tell you which playlists the current song already lives in, so you don't re-sort what's already filed. Chips pick up the song's own accent colour (kept legible on dark album art), and the Loved chip is marked with a ♥. Tap a chip to open a **tracklist sheet** for that playlist — cover hero, monospaced rows with durations, tap-to-preview.
...
- **Settings sheet** — theme (System/Light/Dark), a 33-swatch accent palette (contrast-aware labels), haptics master toggle, auto-play on swipe, hot-clip preview, author-name override for created playlists, read-only-playlist scope toggle, up-swipe Loved-playlist target, a Look section with a status-bar visibility toggle (hidden by default).
- **Insights screen** — a full-screen stats view opened from Settings: hero sort count with a **library-coverage gauge**, current/best streaks, a per-playlist breakdown, a 7-day sorted-vs-dismissed chart, **Top Artists with portraits that tap through to the artist hub**, all-time **genre mix** and **release-decade (Eras) histogram**, keep rate, and a Loved highlight. Everything derives from the durable SwiftData rows plus one library walk — no counters, so it works retroactively over the whole sorting history.
...
**Live on the App Store** — [Culla Music: Swipe Your Songs](https://apps.apple.com/us/app/cullamusic/id6778348600), version 1.5.0 (build 2), submitted 2026-07-27.

MVP plus seven build phases — most recently a Liquid Glass design-language pass and a deliberate restraint pass back toward minimalism, a polish round (richer swipe card, per-playlist library filter, scrubbable progress bar, catalog-track auditioning in the scoped/dismissed decks, sortable Manage Playlists segments, content-shaped loading skeletons, scrub-and-expand hero preview for every source (incl. a picked playlist/artist), iOS 26 glass/mesh fixes), full localization into eight languages, the Insights stats screen (coverage, streaks, taste profile, eras), and a final round of choreography/reliability work (session-transition polish, down-swipe share arming disc, playlist tracklist sheet, History tombstones, dismissed/catalog data-integrity fixes) that shipped alongside the 1.5.0 submission.

On-device QA coverage hasn't kept pace with the last two phases — see the `Projects/Culla-Music/Phases/` and `qa-testing-tracker.md` notes for what's verified and what's still pending.
